...
Например:
    ...
    Объясните проблему, которую решает коммит. Сфокусируйтесь на том, почему вы сделали изменения, а не на том, как (это видно из кода). Может есть побочные эффекты или последствия, которые не видно сразу? Здесь самое место рассказать о них.
